...
38 priority pages completed, in approximate chronological order:
...
- **Tea/soda dose-table framing correction (small, low priority):** Both tea.md and soda.md contain a sentence stating no clean mg/kg caffeine cutoff exists from confirmed sources. This is now known to be imprecise — the coffee hub (§4.1 entry 38) demonstrates that Merck's methylxanthine thresholds do apply to caffeine, just less usefully given serving-level variation by tea type/soda product. One-line correction needed on both pages: reframe to note a dose table exists (cross-link /coffee/) but serving variability makes the type-by-type comparison more practically useful on those specific pages.
- **Chocolate-cluster satellite upgrade (flagged during the hub-upgrade batch, not scheduled):** milk-chocolate, chocolate-chips, cocoa-powder, and hot-chocolate-mix are live-sourced (3 sources each, no dead links) but thin old-format pages, not calibrated satellites of the new chocolate hub. Same opportunity that chives/green-onions/leeks represented relative to onions/garlic before their restoration. Not urgent — these pages aren't broken, just underdeveloped — but a natural next efficient-batch candidate given how recently the chocolate mechanism research was done.
