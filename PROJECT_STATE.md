...
- **Caution** — dose-dependent concern. Real but not categorically toxic. Most pages on the site end up here. Caution can be *severity-driven* (a distinct or serious mechanism, e.g. fat/pancreatitis or salt toxicosis) or *concentration-driven* (an ordinary concern at elevated density, e.g. the sugar in dates) — see §5.14 for the distinction and when to use each framing.
...
62 priority pages completed, in approximate chronological order:
...
46. apple (§6.2 data-driven prioritisation, selected specifically for its zero-clicks-at-strong-position anomaly — position 6.59, 1,732 impressions, zero clicks; the current page's AKC and PetMD sources were both live, an unusually clean citation set requiring no framework/analog attribution; central finding — cyanide from seeds is real in principle but a practical non-issue: Merck's own toxicology confirms crushing/mastication is required to release cyanogenic glycosides, so intact swallowed seeds pass through inertly, and the dose math requires the crushed seeds of roughly 100+ apples to reach a toxic threshold for a dog; the woody core is correctly identified as a separate, genuine mechanical choking/obstruction hazard, kept fully weighted despite the Safe status; status recalibrated Caution → Safe — the site's first clearly-Safe fruit-cluster page, following the same §5.10 transparency discipline as the tea/soda Toxic→Caution moves, with the reasoning stated explicitly that this was a deliberate departure from initial Caution-by-default pattern-matching (avocado precedent) since neither driver (cyanide, sugar/quantity) held up as Caution-specific on close inspection; CTR-optimised metaTitle/description written specifically to test whether title/snippet framing (not content) was the actual lever behind the zero-click pattern — worth checking against the /eggs/ anomaly (§6.2) at the next analytics reading; Category 3 affiliate)
47. ricotta-cheese (§6.2 data-driven prioritisation, selected for the zero-click-at-strong-position pattern — position 6.8, 1,871 impressions, 1 click; analog attribution to the general AKC/PetMD cheese pages, no dead links to clear; positioned precisely on the established dairy-cluster spectrum — low lactose (~2g/quarter-cup, below milk and yogurt), moderate-low fat (10-12% whole/5-7% part-skim, below sour cream's 18-20%) — genuinely one of the gentler dairy options, but no apple-style affirmative-health case, so stays Caution rather than Safe; central "same name, different product" finding — plain fresh ricotta is low-sodium, but ricotta salata (aged, salt-cured) runs ~400mg sodium/oz, a meaningfully different product; allium research deliberately tested and rejected the sour-cream parallel — verified the garlic/onion risk in ricotta dishes (lasagna, stuffed shells) comes from the surrounding sauce, not the ricotta itself, so kept as a proportionate secondary note rather than a central section, with explicit cross-page acknowledgment of why this differs from sour-cream-and-onion dip's direct-mixing finding; sweet ricotta (cannoli, cheesecake) flagged with the cannoli-chocolate-chip caveat; CTR-optimised metaTitle/description — third data point in the eggs/apple/ricotta title-vs-content tracking (§6.2); Category 3 affiliate)
48. deli-meats (§6.2 data-driven prioritisation, highest remaining volume at the time — position 8.47, 3,542 impressions, 7 clicks already converting somewhat; cleared two dead sources (AKC and PetMD deli-meat URLs both 404), replaced with a live AKC Listeria-specific page plus Merck-salt and Merck-allium; direct bacon sibling — reused /bacon/'s fat/pancreatitis and nitrate-calibration-correction findings rather than re-deriving; sodium, not fat, leads here given larger typical servings, calibrated by type (hard salami ~600-960mg/2oz down to lean roast beef ~180-280mg/2oz) against Merck's salt-toxicosis threshold; allium confirmed as a real, direct-in-product finding in salami/pepperoni (garlic/onion powder mixed into the cure itself, not sauce-adjacent) — same tier as coleslaw's later onion finding; central distinctive finding — Listeria risk, with the explicit causal reasoning that deli meat is served ready-to-eat/uncooked (unlike cooked bacon, where heat kills Listeria), sourced to a dedicated AKC page and the 2024 Boar's Head recall, given its own full section; "gentler vs. worse" type guidance included (plain turkey/lean roast beef vs. salami/pepperoni/bologna); Category 1 affiliate — a genuine "what to give instead" scenario, reused bacon's exact Stewart Pro-Treat URL)
49. coleslaw (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 6.28, 1,677 impressions, 3 clicks; cleared a dead AKC coleslaw citation, framework/analog attribution to AKC/PetMD cabbage pages plus Merck-allium and ASPCA; mixed-dish calibration approach — cabbage base genuinely harmless (reused /cabbage/ and /carrots/), the dressing/add-ins are the real concern: onion confirmed as a standard, direct-in-product ingredient (same weight as deli-meats' salami finding, not a sauce-adjacent case), fat/pancreatitis reused from /mayonnaise/ without re-derivation, sugar routed to /sugar/; central finding — raisins are a genuinely common variant ingredient (raisin coleslaw, Waldorf slaw, broccoli slaw, carrot-raisin slaw are standard named recipes) representing a severity-asymmetric escalation from every other concern on the page (dose-dependent caution vs. potential acute kidney failure); handled via prominent in-body structural prominence (dedicated section, explicit severity-asymmetry statement) with the full toxicology deliberately NOT re-cited on-page, routed instead to the already-sourced /raisins/ and /grapes/ hubs — same "alert and route, don't duplicate" discipline as bacon/deli-meats' nitrate cross-linking; vinegar-based variant noted as lower-fat but not risk-free (onion/sugar typically still present); Category 3 affiliate, CTR-optimised metaTitle/description — 4th data point in the eggs/apple/ricotta/coleslaw title-test tracking)
50. buttermilk (§6.2 data-driven prioritisation — 1,772 impressions, position 7.2, 6 clicks (already lightly converting, not a zero-click case, so deliberately NOT added to the §6.2 title-test tracking set); dairy-cluster continuation joining milk, yogurt, sour-cream, evaporated-milk, condensed-milk, ricotta-cheese; cleared a dead PetMD dairy-products URL and dropped a below-first-tier Dial A Vet source, replaced with the sour-cream-pattern 4-set (AKC-milk framework, PetMD-yogurt cultured-dairy analog, VCA-allium, ASPCA); lead is a ricotta-salata-style "same name, surprising product" hook — the "butter" in the name misleads toward richness, but modern store buttermilk is cultured low-fat/skim milk (~1–2% fat, the lowest-fat member of the cluster) with lactose reduced ~a quarter below milk by fermentation, so it lands at the gentle end alongside yogurt; sodium identified as the one axis where buttermilk is "worse" than milk (~180–290 mg/cup vs ~105), given a proportionate note cross-linked to /salt/; cooking-use angle handled honestly as mostly not buttermilk-specific (baked goods and fried-chicken marinade concerns belong to the dish); the ranch/seasoned finding treated as a clearly-proportionate secondary note with the explicit "plain buttermilk contains no allium; ranch is a separate downstream product you make from it" distinction — the ricotta-lasagna (sauce-adjacent) tier, deliberately NOT inflated to the sour-cream/coleslaw direct-in-product tier; powdered/dried buttermilk noted briefly (ranch seasoning packets carry the allium powder); status Caution, correctly not reaching Safe on the ricotta precedent (individual lactose tolerance, elevated sodium, no affirmative-health case); CTR-optimised metaTitle/description as standard; Category 3 affiliate, consistent with sour-cream/ricotta and the "no probiotic redirect outside yogurt's specific framing" precedent)
51. capers (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 6.95, 1,215 impressions, 2 clicks; 5th data point added to the §6.2 eggs/apple/ricotta/coleslaw title-test tracking set, a genuine fit for the pattern unlike buttermilk; cleared two dead sources (AKC and PetMD capers-specific URLs both 404 — no dedicated caper coverage exists on either), framework/analog attribution to the AKC/PetMD pickles pages as the brined-condiment analog plus ASPCA and Merck-salt for the sodium figures; deliberately a low-drama, shorter page — no toxin in the caper bud itself, and the calibration hinges on the "intense by concentration, trivial by serving" sodium math (a single caper ~8 mg sodium, a few off a plate ~40 mg, all far below Merck's ~2–3 g/kg salt-toxicosis threshold), with the small-dog-into-a-whole-jar case (~2,300 mg sodium per 100 g jar) as the one proportionate scenario rather than a fear-section; status Caution, NOT Safe — the reasoning deliberately led with the separable-vs-inherent-hazard distinction (apple reached Safe because its core/seeds are a removable prep step on a clean food, whereas capers' salt is inherent to the commercial product — there's no "plain caper" sold that isn't brined or salt-packed), with sibling consistency to /pickles/ and /olives/ as a secondary note rather than the primary justification; vinegar-in-brine handled as a brief GI-only footnote, salt-packed-vs-brined as a one-sentence distinction; dishes (piccata's butter/wine, tapenade's garlic) framed proportionately as "that's the dish, not the caper"; Category 3 affiliate, 3 links (panel + a body callout deliberately reworded and re-verified at ship time to anchor specifically to the jar-raid scenario rather than implying general caper risk — caught and fixed during build verification, not draft review, a useful instance of the review discipline operating one stage later than usual); CTR-optimised metaTitle/description as standard)
52. feta-cheese (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 9.25, 1,349 impressions, 1 click; 6th data point in the §6.2 title-test tracking set; cleared two dead sources (AKC and PetMD feta-specific URLs both 404), analog attribution to the AKC/PetMD cheese pages (same pattern as ricotta) plus VCA-allium, ASPCA, and Merck-salt — a 5-source set justified because feta uniquely has both a central salt story and a central allium story, each earning its own specialist anchor (same 5th-source reasoning as garlic); dairy-cluster continuation positioned at the salty pole, explicit bookend to buttermilk's gentle pole; transparent fact-correction per §2.6 — the previous page's "high in lactose" framing was wrong, feta is actually fairly low-lactose (brining/aging), recalibrated the concern order to salt ≫ fat > lactose; sodium is the lead (~312 mg/oz, ~3× milk, alongside ricotta salata's ~400 mg/oz), calibrated against Merck's ~2–3 g/kg salt-toxicosis threshold and explicitly tied into the now-named "inherently-salty" trio with ricotta-salata (dairy sibling) and /capers/ (non-dairy sibling), reusing capers' separable-vs-inherent framing ("you can core an apple, but you can't de-salt a feta"); fat secondary (~21%, near sour-cream); two genuine allium findings given real weight — garlic-herb feta as a direct-in-product exposure (named Athenos as the verified widely-sold commercial example, sour-cream-dip tier) and Greek salad's red onion as a direct-in-dish exposure (standard traditional ingredient, coleslaw tier); spanakopita brief/dish-adjacent; watermelon-feta included as a deliberate calmer inversion note (the fruit's dog-safe, the cheese is the only concern); status Caution; Category 3 affiliate, 3 links standard placement; CTR-optimised metaTitle/description as standard)
53. artichoke (§6.2 data-driven prioritisation — ~3,153 combined impressions, but notably the page whose Search Console data motivated the canonical/trailing-slash infrastructure fix shipped just before this rewrite — see §5.13; deliberately excluded from the §6.2 zero-click title-test tracking set because its historical click data is contaminated by the URL split, which would conflate the trailing-slash fix with any title-test effect; cleared two dead sources, PetMD dropped entirely per §5.9 — no live artichoke or general-vegetable PetMD page exists; AKC used as openly-stated general fruit-and-vegetable framework, plus ASPCA, VCA-allium, and Merck-salt; status Caution, with the reasoning made explicit and reader-facing as the inverse of apple — apple reached Safe because its recommended form (cored slices) is also the typical real-world encounter, whereas artichoke's clean form (plain steamed heart) is what careful preparation looks like but is the minority of encounters, with marinated jarred hearts and spinach-artichoke dip as the dominant real-world forms; mechanical hazard (tough bracts + fibrous choke) given real weight as a larger inedible fraction than apple's core; preparation-dependent garlic handled with the explicit "that's the butter/dip, not the artichoke" dish-adjacent framing; spinach-artichoke dip given its own full section as the richest context; metaTitle deliberately kept self-contained (not referencing the apple comparison) since methodological cross-references belong in body prose, not titles that need to work for readers with zero site context; Category 3 affiliate, 3 links standard placement; first content page to ship into the corrected canonical/trailing-slash structure)
54. brown-sugar (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 7.54, 1,508 impressions, 1 click; 7th data point in the §6.2 title-test tracking set, a clean fit unlike artichoke; efficient sugar-cluster continuation — the basic sugar/dental/diabetes/obesity calibration deliberately not re-derived, cross-linked heavily to /sugar/ instead; cleared two dead sources (AKC can-dogs-eat-sugar and VCA human-foods-should-not-eat, both 404), replaced with AKC-human-foods framework, live PetMD-sugar retained, plus VCA-xylitol and ASPCA; molasses handled as an honest non-issue — trace minerals nutritionally negligible at dog-relevant amounts, brown sugar neither better nor worse than white, resisting the "molasses is healthier" health-halo; the one genuine finding is a proportionate xylitol note on sugar-free/keto "brown sugar substitutes" — the maple-syrup "sugar-free version is the real hazard" precedent, cross-linked to /xylitol/ and /maple-syrup/, but deliberately smaller in scope (nicher keto-baking product) and with no specific brand named since none could be verified as currently xylitol-sweetened (major brands are erythritol) — "category is mixed, check the label" framing stated transparently on the page itself; dish contexts brief, all "that's the other ingredient" framing (baked goods, cinnamon treats, brown-sugar-glazed ham); status Caution consistent with /sugar/; Category 3 affiliate, 3 links standard placement)
55. swiss-cheese (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 8.47, 980 impressions, 0 clicks; 8th data point in the §6.2 title-test tracking set; efficient cheese-cluster continuation reusing ricotta/feta/general-cheese groundwork; cleared two dead sources (AKC Swiss-specific and VCA lactose-intolerance URLs both 404), analog attribution to AKC/PetMD cheese pages plus VCA-Pancreatitis (chosen over Merck-salt since the concern here is fat, not salt) and ASPCA; positioned as the gentlest cheese in the cluster on lactose and sodium — near-zero lactose (aged cheese, ripening bacteria consume the milk sugar) and low sodium (~55 mg/oz, roughly one-sixth of feta's ~312), with the low-lactose finding noted as an earned human-to-dog transfer (same mechanism), unlike sour-cream's rejected probiotic angle; explicit contrast to the "inherently-salty" trio (feta/ricotta-salata/capers) as the low-sodium counterpoint; status Caution, with the reasoning sharpened into a standing, reader-facing principle — two axes resolving to near-zero (lactose, sodium) doesn't cancel a third, independently serious concern (fat/pancreatitis) — explicitly distinguished from apple's Safe rating, where every axis genuinely resolved and nothing independently serious remained; this is now the second page (after capers) to correct its reasoning from cluster/sibling-consistency toward a sharper, standalone principle; fondue handled as one brief, proportionate dish-adjacent note (garlic + wine); Category 3 affiliate, 3 links standard placement)
56. dates (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 9.99, 1,707 impressions, 0 clicks; 9th data point in the §6.2 title-test tracking set; cleared one dead source (PetMD) while keeping the live direct AKC dates page, plus PetMD-sugar, Merck grape/raisin/tamarind toxicosis, and ASPCA; headline correction — dates (palm family) are botanically unrelated to grapes (Vitis), and the grape/raisin toxin is specific to that plant lineage, so it doesn't transfer to dried fruit generally — given unmissable prominence (shortAnswer opening, dedicated section, standout FAQ), same "internet conflates X with Y" family as capers/lemon; the pit verified as purely mechanical (no cyanide — the Prunus cyanogenic pattern doesn't transfer to the palm-family date pit), pitted is the standard commercial form, cross-linked to /apple/ as the parallel; establishes a new standing pattern — concentration-driven Caution (sugar density, ~66% vs. apple's ~10-15%) explicitly distinguished from severity-driven Caution (fat/pancreatitis, salt) — see new §5.14; date syrup/sugar noted as having no sugar-free/xylitol substitute category to worry about, unlike maple-syrup; energy/protein balls as a genuine dish-adjacent finding (chocolate + macadamia); Medjool-vs-Deglet-Noor practical sizing note; status Caution; Category 3 affiliate, 3 links standard placement)
57. pizza-roll (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 8.52, 1,100 impressions, 0 clicks; 10th data point in the §6.2 title-test tracking set; cleared one dead source (AKC pizza-specific URL 404), kept live PetMD-pizza, plus AKC-human-foods framework, VCA-Pancreatitis, and ASPCA — 4-set; deliberately leads the page (before sodium/fat/allium, ahead of the site's usual "reassure or calibrate, then build the case" ordering) with the pizza-dough Emergency distinction, given genuine emergency-adjacency — a reader whose dog ate actual raw dough could plausibly land here first; states the specific verified mechanism (factory-baked shell, no live yeast activity, no fermentation gas or ethanol production) rather than asserting reassurance, with an explicit redirect to /pizza-dough/ for real raw-dough cases; status recalibrated Toxic → Caution per §5.10 — the old page's "pizza is toxic" framing corrected to the real, dose-dependent processed-food concerns; verified major-brand (Totino's) ingredients directly rather than assuming — garlic powder confirmed present in filling/seasoning, onion powder confirmed NOT present (corrected an initial assumption); salt/fat/allium mechanisms routed to hub cross-links (/bacon/, /deli-meats/, /garlic/) rather than re-derived; quantity consideration (a couple vs. a whole plate) reuses the bacon/deli-meats cumulative-load framing; Category 1 affiliate, Stewart Pro-Treat substitute — the canonical URL was pulled verbatim from bacon.md rather than shipped as an unverified reconstruction, after the reconstructed ASIN was found to be incorrect during pre-ship verification; see the §5.7 addendum this incident produced)
58. parmesan-cheese (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 10.27, 1,243 impressions, 0 clicks; 11th data point in the §6.2 title-test tracking set; cheese-cluster continuation; near-zero lactose (extensive aging, same pattern as swiss-cheese) but the highest sodium of any cheese in the cluster (1,529 mg/100g — above feta's 1,116); status Caution via a new principle distinct from §5.14 — composition determines the badge, not typical serving habit (see new §5.15); "parmesan is normally used in small sprinkled amounts" describes a human cooking pattern, not a property of the cheese, and doesn't reliably describe a dog's realistic exposure to a wedge, a shredded bag, or a bowl left on the counter; practical guidance still reflects that an ordinary sprinkle is low-risk — the badge and the guidance aren't in tension, they're answering different questions; rind given its own clear mechanical-hazard section (apple-core/artichoke-choke tier — hard, inedible, genuine choking/obstruction risk); garlic-pasta context noted briefly as dish-adjacent, not cheese-specific; metaTitle explicitly compresses the whole finding into "the cheese is saltier than it feels"; Category 3 affiliate, 3 links standard placement)
59. chestnuts (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 11.05, 1,384 impressions, 1 click; cleared dead sources, 4-set including Merck for the aesculin/horse-chestnut distinction; status Safe, mapped with full structural confidence onto the apple precedent (not a hedged or approximate fit) — two hazards (raw-chestnut tannin/digestibility, shell/pellicle) both fully resolved by standard preparation (roasting) that is already the near-universal norm, nothing independently serious remaining, same two-part test apple passed (hazard fully resolves via ordinary prep; the clean form is also the typical real-world form); genuinely low-fat relative to other tree nuts, positioned as an explicit future reference point if almonds/walnuts/macadamias get full rewrites; the page's most valuable contribution is the sweet-chestnut (Castanea) vs. horse-chestnut/buckeye (Aesculus, genuinely toxic — aesculin) naming-confusion warning, deliberately given early, unmissable structural placement — analogous in priority to pizza-roll's Emergency-distinction placement, though calibrated to a lower tonal register since the underlying failure mode (reassurance misapplied to the wrong plant) is the shared risk, not comparable severity; holiday-stuffing garlic/onion note included brief and dish-adjacent; Category 3 affiliate)
60. soy-milk (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 9.94, 1,163 impressions, 2 clicks; cleared dead sources, 4-set (AKC-yogurt framework, PetMD-plant-milk, VCA-xylitol, ASPCA); the fuller dedicated plant-milk treatment, with /milk/'s existing brief plant-milk section now pointing here for depth; status Caution — gentle plain default (no toxin, soy a common dog-food ingredient) with genuine sweetened/flavoured-variant and chocolate-variant cautions; soy allergy noted as a distinct practical point, kept separate from the toxicology discussion rather than conflated with it; thyroid/isoflavone question deliberately omitted — no canine-specific evidence exists to responsibly discuss it either way, correctly treated as a silence-is-honest case rather than a manufactured hedge; affiliate deliberately kept Category 3 standing rather than pivoting to yogurt's FortiFlora probiotic framing — the reasoning explicitly distinguishes reader intent: soy-milk readers are checking a specific human food already in the fridge, not choosing a substitute the way bacon/broth readers are, so forcing a probiotic recommendation here would be the kind of surface-level "both about digestion" pattern-matching the site has specifically avoided; Category 3 affiliate)
61. corn-flakes (§6.2 data-driven prioritisation, zero-click-at-strong-position pattern — position 8.38, 1,304 impressions, 1 click; cleared dead sources, 4-set; status Caution, driven by sugar content in sweetened/frosted variants (cross-linked to /sugar/) rather than the plain product, which is genuinely low-concern; two findings deliberately and correctly omitted after explicit dose-math/evidence checks rather than silent assumption — iron fortification (ruled out via dose math; a realistic serving comes nowhere near the iron-supplement-overdose threshold that's the actual veterinary concern) and BHT/preservatives (no canine-specific veterinary evidence exists to responsibly cite either way, same silence-is-honest discipline as soy-milk's thyroid/isoflavone omission); genuine, honestly-scoped adjacent finding — raisin-containing cereals (Raisin Bran and similar) flagged as a real cereal-aisle risk, explicitly framed as not corn-flakes-specific rather than forced into false relevance, cross-linked to /raisins/; sugar-free/diet variant xylitol check per established discipline; milk-eating-context kept to a brief cross-link; Category 3 affiliate)
62. donuts (§6.2 data-driven prioritisation — combined ~2,414 impressions across two URL variants, a residual trailing-slash duplication artifact from before the §5.13 canonical/redirect fix; deliberately excluded from the §6.2 title-test tracking set per the artichoke precedent, confounded historical click data; cleared two dead sources, 4-set (AKC-human-foods framework, VCA-Pancreatitis, Merck-chocolate, ASPCA) anchoring the two most serious mechanisms on-page while routing fat/sugar/xylitol/raw-dough to their hubs (/bacon/, /sugar/, /xylitol/, /pizza-dough/); status Caution — straightforward multi-factor processed-junk-food pattern (fat + sugar on every donut, chocolate/xylitol on variants), same family as pizza-roll/deli-meats, no recalibration needed; chocolate donuts given real weight as a genuine theobromine exposure, judged consistently by amount and dog size across every section; two deliberate structural decisions — (1) the raw-dough distinction placed as a clear mid-page section rather than leading, since "donut" doesn't carry pizza-roll's acute linguistic conflation with actual raw dough; (2) Category 1 affiliate with a reframed callout that names the real appeal being served (inclusion in the sharing moment) rather than implying flavor-equivalence between the savoury Stewart substitute and a sweet food — resolving the mismatch by acknowledging it rather than papering over it; 5-link structure (panel + body callout for both Stewart and ARCA, panel-only for Forever Dog), Stewart/ARCA/Forever Dog URLs pulled verbatim from bacon.md per §5.7)
...
**Addendum (July 2026, pizza-roll incident):** A reconstructed Stewart Pro-Treat URL was flagged as unverifiable during drafting, then approved for shipping "as-is" in a subsequent review that didn't have full visibility into the earlier flag. Rather than ship the flagged reconstruction, the correct discipline was applied: pull the canonical URL verbatim from a known-good page (bacon.md) instead, and report the resolution back rather than silently shipping either the flagged version or a silent fix. The reconstructed ASIN was subsequently confirmed incorrect (B0002DK9L0 vs. the correct B0D2PN4C6Y) — had it shipped, it would have been a live affiliate link to the wrong product.

**Standing rule:** when a broad "approved as-is" instruction collides with a previously-flagged, unresolved uncertainty (particularly around affiliate URLs, where the cost of being wrong is a live, real-world broken or incorrect link), the flagged uncertainty takes precedence over the blanket approval. Resolve using the safer, verified option — a known-good page is the default source, not a fresh reconstruction — and report the resolution explicitly in the same message as the ship confirmation, so the deviation from the literal instruction is visible and reviewable, not silent.
...
### 5.12 Logging discipline: §4.1 entries must ship in the same exchange as the page

Two documented instances of this gap (lemon/tortillas, June 2026; deli-meats, July 2026): a page ships and its §4.1 entry gets deferred to "the next documentation pass," but the next pass is often triggered by something unrelated and doesn't naturally prompt a check for missing prior entries. The gap then sits undetected until a later count-reconciliation catches it.

**Rule: when a page is approved for writing to disk, the §4.1 entry text should be drafted in the same exchange, even if the actual PROJECT_STATE.md commit happens later.** This doesn't mean every single-page ship needs an immediate PROJECT_STATE.md commit — batching documentation updates across a few pages is fine and efficient — but the entry text itself should exist and be reviewed at ship time, not reconstructed retroactively from the live page when a gap is eventually noticed.

### 5.13 Site-wide canonical tags and trailing-slash consistency (July 2026)

Diagnosed when artichoke's Search Console data showed impressions split across two URLs (/artichoke and /artichoke/, combining to one page's worth of real traffic). Investigation found no actual duplicate-content bug — Cloudflare Pages correctly 301-redirects the no-slash form to the trailing-slash canonical, site-wide and consistently. The split was a signal-dilution issue from two contributing causes:

1. **No `<link rel="canonical">` tag existed anywhere on the site** — zero canonical tags across all templates, leaving Google to consolidate on the 301 alone with no explicit hint.
2. **~1,072 internal links (≈28% of all internal links) pointed at the no-slash form**, generated by several templated components (most significantly the related-foods widget appearing on all ~275 pages) that built hrefs from slugs without appending a trailing slash. Every crawl of these links re-surfaced the no-slash URL in Google's index, slowing consolidation.

**Fix applied (one dedicated commit, ahead of the artichoke content rewrite):**
- Self-referencing `<link rel="canonical">` tags added to 8 of 9 page templates (all indexable templates; deliberately excluded from 404.astro, which carries `noindex` — a canonical on a noindex page sends contradictory signals).
- All 7 templated no-slash link generators corrected to trailing-slash form (one more than initially scoped — a category-link generator at `[...slug].astro:118` was found and fixed during implementation, since leaving it would have continued feeding Google no-slash category URLs from every food page).
- `trailingSlash: 'always'` added to `astro.config.mjs` as a standing guard against future regressions.

**Result:** 0 remaining no-slash internal links anywhere in dist (down from 1,072). Severity of the original issue was assessed as low/cosmetic — the 301 already prevented genuine duplicate-content penalties — but the fix improves indexing consolidation speed and is now permanent, template-level protection rather than a one-off patch. Artichoke (§4.1 entry 53) was the first content page to ship into the corrected structure; its own historical CTR data is excluded from the §6.2 title-test experiment as a result (see entry 53).

### 5.14 Concentration-driven vs. severity-driven Caution

Surfaced during the dates rewrite (July 2026), when the status reasoning didn't cleanly map onto either of the site's existing borderline-calibration patterns (apple's "every axis resolves, badge reflects the standard form" Safe call, or swiss-cheese's "one independently-sufficient serious concern overrides two resolved ones" Caution call).

Dates required a third category: an ordinary, familiar concern (fruit sugar — the same thing present on nearly every fruit page) present at meaningfully elevated concentration (~66% by weight, roughly six times apple's), rather than a distinct or categorically serious hazard. This is Caution for a different reason than fat-driven Caution (bacon, swiss-cheese, sour-cream — genuine pancreatitis risk) or salt-driven Caution (feta, capers, ricotta salata — sodium toxicosis mechanism). Nothing about date sugar is dangerous in the way those mechanisms are; it's simply concentrated enough that portion size becomes a genuine, ongoing consideration rather than a background detail.

**Standing principle:** when a status call hinges on an ordinary concern (sugar, fat, sodium — the site's recurring "background" categories) present at unusually high concentration, rather than on a distinct or severe mechanism, state this explicitly as concentration-driven Caution — both internally, when deciding the calibration, and reader-facing, on the page itself ("this is a caution about density, not danger"). Don't borrow the language used for severity-driven Caution pages (e.g. swiss-cheese's "two green lights don't cancel a red one," which implies a red light — a categorically serious remaining concern — exists). The distinction matters because it tells a repeat reader something genuinely useful: not every Caution badge on this site means the same level of concern, and being explicit about which kind applies is more honest and more useful than a uniform badge with no texture.

**Likely to recur:** other dried or concentrated foods (dried apricots, dried mango, and similar) may fit this same pattern — an otherwise-ordinary fruit concern at elevated density, distinct from any toxin or severe mechanism.

### 5.15 Composition determines the badge; typical serving size doesn't override it

Surfaced during the parmesan-cheese rewrite (July 2026). Parmesan is the most sodium-concentrated cheese the site has covered (1,529 mg/100g, above feta's 1,116), but it's also the cheese most commonly used in the smallest typical servings — a sprinkle, not a slice. This created a genuine tension the site hadn't faced before: every prior borderline case (apple, artichoke, swiss-cheese, dates) had concentration and typical serving size pointing the same direction. Parmesan is the first page where they diverge — extremely concentrated by weight, typically served in tiny amounts.

**The resolution:** the status badge tracks what the food *is* (its composition), not how a typical person happens to use it. "Usually used sparingly" is a fact about common human cooking habits, not a property of the food itself, and it doesn't reliably describe how a dog would actually encounter the product — a wedge left within reach, a bag of shredded cheese, a bowl on the counter. The badge has to serve the reader whose situation doesn't match the "typical, careful" pattern, not just the one who does.

This is distinct from §5.14 (concentration-driven vs. severity-driven Caution), which addresses *why* something is Caution once concentration is established as the relevant factor. §5.15 addresses a different question: *whether typical serving size can downgrade a badge that composition alone would otherwise justify.* It can't — composition sets the badge; typical-use context belongs in the practical "what to do" guidance (where it's genuinely useful — a normal sprinkle really is low-risk) rather than in the status classification itself.

**Standing principle:** when a food is highly concentrated on some axis but conventionally used in small amounts, don't let "but people don't usually use much of it" downgrade the Caution/Toxic classification. State the composition-based reasoning plainly and reader-facing, then give practical guidance that reflects realistic typical use separately — the reader gets both the honest badge and the reassurance that ordinary use is fine, without the two being in tension.

**Likely to recur:** other concentrated ingredients typically used in small amounts — extracts (vanilla, almond), concentrated spice pastes, bouillon/stock concentrate, and similar "a little goes a long way" products may fit this same pattern.
...
**Priority list exhausted (July 2026).** All candidates from the last full Search Console pull (dates, pizza-roll, parmesan-cheese, chestnuts, soy-milk) have now shipped. A fresh pull is needed before selecting the next target — see §6.2's standing methodology for the pull process.

**Anomaly tracking — zero-click-at-strong-position pages:** Twelve data points now. /eggs/ (§4.1 entry 21, fully rewritten, no title/meta fix applied) remains the control case — still at 0 clicks as of the most recent full reading despite ~7,300 impressions at a strong position (~7.5-7.7). /apple/ (entry 46), /ricotta-cheese/ (entry 47), /coleslaw/ (entry 49), /capers/ (entry 51), /feta-cheese/ (entry 52), /brown-sugar/ (entry 54), /swiss-cheese/ (entry 55), /dates/ (entry 56), /pizza-roll/ (entry 57), /parmesan-cheese/ (entry 58), and /soy-milk/ (entry 60) all shipped with deliberately CTR-optimised metaTitles/descriptions as test cases. /buttermilk/ (entry 50), /artichoke/ (entry 53), and /chestnuts/ (entry 59) remain deliberately excluded — buttermilk was already lightly converting; artichoke's historical click data is confounded by the URL-duplication bug fixed in §5.13; and chestnuts sat at a genuinely weak position (11.05, page-2 borderline) where near-zero clicks is plausibly a position effect rather than a title/snippet effect. Note that chestnuts' exclusion is a *position* call, not a status one — Safe pages are not excluded on principle (apple is Safe and is a core test case); soy-milk (Caution, position 9.94, 2 clicks) was included because it genuinely fits the strong-ish-position/low-click pattern, comparable to dates (9.99) and parmesan (10.27) and consistent with capers (also 2 clicks).

**Interim read (July 2026, most recent full reading):** genuinely mixed, not yet conclusive. Apple — the oldest test case, with the most time to be re-crawled — is still at 0 clicks, which doesn't support the hypothesis. Coleslaw and capers each showed modest upticks (0→3 and 0→2 clicks respectively), but at this scale the difference could easily be noise rather than signal. This reading period also coincided with a large site-wide indexing surge (279→323 indexed pages in two weeks, driven by the citation-cleanup and canonical/trailing-slash work), which makes it hard to isolate a small title-specific effect from the much larger structural changes happening simultaneously. Recommendation: hold off on concluding either way until a reading taken after the indexing surge settles, with more data across all seven test pages.
...
- **Emergency-flag visual pattern for mixed-dish pages (flagged during coleslaw, not implemented):** Coleslaw's raisin finding needed strong visual prominence for a single add-in that changes a page's risk category from dose-dependent caution to a potential emergency. Handled via prose structure alone (dedicated section, explicit severity-asymmetry statement) since no verified CSS class exists for this pattern. Could recur on similar mixed-dish pages (e.g., a future trail-mix, granola, or baked-goods page where raisins, chocolate, or xylitol might be an add-in rather than the headline ingredient). Worth a properly-designed, tested CSS pattern if this need recurs — not worth a one-off unverified implementation.
- **Two hub pages have dead citations, found during recent research (July 2026):** /sugar/ still cites a dead AKC can-dogs-eat-sugar/ URL (404, found during brown-sugar research); /cheese/ still cites a dead VCA lactose-intolerance-in-dogs URL (404, found during swiss-cheese research). Both hub pages are cross-linked heavily by their respective clusters (sugar cluster: brown-sugar, maple-syrup; cheese cluster: ricotta-cheese, feta-cheese, swiss-cheese), so worth a single future citation-repair pass covering both rather than two separate small tasks.
...
### 6.7 First-party digital product initiative (parked, brief received July 2026)

A digital product brief was received proposing "The Dog Safety Reference" — a $15 PDF emergency/preparedness reference (fridge chart, triage flow, vet-call script, symptom reverse index, household audit, safe-alternatives index, seasonal hazards) sold via Lemon Squeezy, funded by free-lead-magnet email capture, run as a 2-week-effort validation instrument. Its stated purpose is not primarily revenue but establishing a conversion baseline (sales ÷ sessions on CTA pages, 90-day window) that would decide whether to build paid products across the wider portfolio (including the directory sites).

**Key discussion points, for whoever picks this up next:**

- **Sample-size concern (the main open issue):** at current traffic (~400 organic sessions/month), a 90-day test yields roughly 1,200 sessions on best-case "top 20 CTA pages" routing — meaning the decision thresholds translate to single-digit sales counts (roughly 1-2 sales at the "rework" line, 4-9 at "working," 8+ at "strong"). This is too small a sample for a single reading to be fully decisive; a handful of anomalous purchases could swing the read either way. Recommendation discussed: don't treat one 90-day reading at current scale as final — build the free lead magnet and email list now (valuable regardless of scale), but delay starting the clock on the *paid-product* decision window until traffic is meaningfully higher, or treat the first reading as directional and require a confirmatory second window before acting on high-stakes outcomes (building a course, applying the model portfolio-wide).
- **Three paths considered:** (A) proceed with the full brief now, accepting wide error bars on the first reading; (B) build the free lead magnet + email capture now, defer the paid-product launch/clock to a later traffic milestone; (C) defer the whole initiative until a traffic threshold, focus entirely on continued page-rewrite momentum. Leaning toward (B) as of this discussion, not yet decided.
- **Production-quality bar:** the PDF's severity/dose/triage content should get at least pizza-dough-Emergency-page-level review rigor, arguably more, since a downloaded/printed PDF can't be silently corrected the way a webpage can. Apply the same research → calibration review → draft → verification cycle used for the 55+ rewritten pages, pointed at the PDF's tables specifically.
- **Efficiency opportunity:** the symptom reverse index (flagged in the brief as highest-value, hardest-to-fake) should be built primarily by mining and cross-referencing already-researched, sourced, dose-calibrated content from the site's existing toxin hubs (allium family — onions/garlic + 3 satellites; chocolate/caffeine family — chocolate/coffee + 4 satellites; xylitol) rather than fresh research from scratch.
- **Product-family framing:** this need not be a single product. The strongest complementary idea discussed is cheap seasonal hazard mini-guides (Christmas/Thanksgiving/Easter/Halloween), which reuse existing sourced content (chocolate, xylitol, grapes, onions, turkey bones), target calendar-predictable demand spikes distinct from the main reference's "just had a scare" trigger, and could run at a lower price point ($3-5) or as an alternate lead magnet. Weaker/more speculative extensions considered and not prioritised: an interactive/searchable version of the symptom index (real build-complexity jump, a 12+-months-out idea if the PDF validates); breed-size or new-puppy-owner variants (risks diluting the site's specific toxicology-focused trust asset); physical products (explicitly conflicts with the brief's own anti-goal of file-only delivery).
- **Email-list tooling recommendation:** MailerLite, free tier, for the lead-magnet and sequence phase — its free tier includes automation up to 1,000 subscribers, unlike Kit (formerly ConvertKit), whose free tier requires a paid upgrade (~$29/month) just to run automated sequences. Neither has a native Lemon Squeezy integration; connecting purchase events to email tagging will need Zapier (~$20/month) or a custom webhook once the paid product is actually live — not needed for the free-lead-magnet phase.
- **Lemon Squeezy fee confirmed current** (verified July 2026): 5% + $0.50 per transaction as merchant of record, no monthly fee on the base tier, handles VAT/sales tax — matches the brief's assumption.

**Status: parked, not started.** No content written, no accounts created, no PDF work begun. Revisit when either (a) traffic scale addresses the sample-size concern, or (b) Patrick decides to proceed anyway and accept the wider error bars on an early reading.
