...
46 priority pages completed, in approximate chronological order:
...
**Anomaly tracking — zero-click-at-strong-position pages:** /eggs/ (fully rewritten, §4.1 entry 21) showed 7,217 impressions at position 7.51 but zero clicks as of the July 21 reading — still unresolved, no metaTitle/description fix applied yet. /apple/ (§4.1 entry 46) was selected specifically to test this pattern and shipped with a deliberately CTR-optimised metaTitle/description as the fix. Check both at the next analytics reading: if apple's clicks improve meaningfully while eggs stays flat, that confirms title/snippet framing (not content) as the lever for this failure mode, and eggs should get the same metaTitle/description treatment without a full content rewrite.
